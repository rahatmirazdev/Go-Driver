...
const MyBookings = () => {
  ...
  const confirmModifyBooking = async () => {
    try {
      ...
      await axios.put(`/bookings/${selectedBooking._id}`, updatedBooking);
      setBookings(bookings.map((booking) => (booking._id === selectedBooking._id ? updatedBooking : booking)));
    ...
    return (
      <div className="container mx-auto px-4 py-16">
        ...
        <div className="text-center">
          <p className="text-lg text-dark-gray mb-4">No bookings found.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-4 py-16 min-h-screen">
        ...
        <table className="min-w-full bg-white">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className="py-2 px-4 border-b font-bold">Booking Date</th>
            ...
            {bookings.map((booking, index) => (
              ...
              >
                <td className="py-2 px-4 border-b">
                  <img src={booking.photoUrl} alt={booking.carModel} className="w-20 h-20 object-cover" />
                </td>
                <td className="py-2 px-4 border-b">{booking.carModel}</td>
                <td className="py-2 px-4 border-b">{new Date(booking.bookingDate).toLocaleDateString()}</td>
                <td className="py-2 px-4 border-b">${booking.dailyRentalPrice}/day</td>
      ...
      {showModifyModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
          <div className="bg-white p-8 rounded-lg shadow-lg max-w-lg w-full">
            <h3 className="text-2xl font-bold mb-4">Modify Booking Date</h3>
            <div className="mb-4">
