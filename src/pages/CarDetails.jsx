...
const CarDetails = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-16">
      <h2 className="text-4xl font-bold text-center text-dark-gray mb-12">{car.carModel}</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div>
          {car.photoUrl ? (
            <img src={car.photoUrl} alt={car.carModel} className="w-full h-64 object-cover mb-4" />
          ) : (
          ...
          <div className="grid grid-cols-3 gap-2">
            {car.images && car.images.slice(1).map((image, index) => (
              <img key={index} src={image} alt={`Car ${index}`} className="w-full h-32 object-cover" />
            ))}
          </div>
        </div>
        <div>
          <p className="text-lg text-soft-green mb-2">${car.dailyRentalPrice}/day</p>
          <p className="text-sm text-gray-500 mb-2">{car.availability}</p>
      ...
      <div className="mt-8">
        <h3 className="text-2xl font-bold text-dark-gray mb-4">Reviews</h3>
        {car.reviews && car.reviews.length > 0 ? (
          car.reviews.map((review, index) => (
            <div key={index} className="mb-4">
              <div className="flex items-center mb-2">
                <img src={review.profileImage} alt={review.name} className="w-8 h-8 rounded-full mr-2" />
                <h4 className="text-lg font-semibold text-dark-gray">{review.name}</h4>
              </div>
              <div className="flex mb-2">
                {Array.from({ length: review.rating }).map((_, i) => (
                  <FaStar key={i} className="text-soft-green" />
                ))}
              </div>
      ...
      <div className="mt-8">
        <h3 className="text-2xl font-bold text-dark-gray mb-4">Add a Review</h3>
        <textarea
          value={reviewText}
        ...
        <div className="flex items-center mb-4">
          ...
          {Array.from({ length: 5 }).map((_, i) => (
            <FaStar
              key={i}
              className={`cursor-pointer ${i < rating ? "text-soft-green" : "text-gray-400"}`}
